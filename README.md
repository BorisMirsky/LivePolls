...
-------------------------------------------------------------------------
                      ...
                      Проект будет состоять из нескольких слоев: 

- API (контроллеры и хабы SignalR), 
-- REST API для управления опросами (создание, редактирование, закрытие). Используются контроллеры.
-- SignalR Hub для реального времени: PollHub. Он будет обрабатывать подключения клиентов, подписки на группы (комнаты) опросов, получение голосов и рассылку обновлений.
